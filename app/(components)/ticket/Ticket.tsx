...
export default function Ticket({data}) {
    const {removeTicket, modifyTicket} = useContext(TicketsContext);
    ...
    const handleModify = async () => {
        const updatedData = {...data, price: finalizedPrice};
        await modifyTicket(data.id, updatedData);
    ...
    return (
        <main className="flex flex-col h-full">
                ...
                <div className="flex w-full max-w-3xl text-zinc-900 h-64">
                    ...
                    <div className="h-full bg-white flex items-center justify-center px-8 rounded-r-3xl">
                        <button onClick={handleModify}
                            ...
                            <div
                                className="w-32 h-32 bg-gray-300 flex items-center justify-center px-8 rounded">Finalize
                                Price
                            </div>
                        </button>
                    </div>
            ...
            {isFutureTicket() && (
                <button onClick={handleDelete} className="bg-red-500 text-white px-4 py-2 rounded">Ticket
                    zurückgeben</button>
            )}
        </main>
    );
