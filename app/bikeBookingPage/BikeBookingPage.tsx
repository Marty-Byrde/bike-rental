...
import {createTicket} from '@/app/(components)/ticket/ticketAPI';
...
export default function BikeBookingPage() {
    ...
    const [adultTicket, setAdultTicket] = useState(false);
    ...
    const calculatePrice = (model, category, date, time, station) => {
        ...
        if (rentingOption === 'future') {
            ...
            const rentalEnd = new Date(rentalStart.getTime() + 60 * 60 * 1000);
    ...
    const handleBooking = async () => {
        ...
        childrenDetails.forEach(async (child) => {
            ...
            await createTicket(childTicket);
        });
    };

    // Hier würde der Rest der Benutzeroberfläche für die BikeBookingPage folgen...
    // Dies könnte Formularfelder für die Eingabe von Ticketdetails enthalten,
    // sowie Schaltflächen zum Senden der Daten.
